...
async function executeReadFile(params: {file_path: string}): Promise<string> {
    ...
    try {
        const stat = await fs.stat(securePath)
        if(stat.isDirectory()) {
            return `Error: ${params.file_path} no es un archivo.`
        }

        if(stat.size > MAX_FILE_SIZE){
            return ` Archivo demasiado grande (${stat.size} bytes)` +
                    `Considera leer una sección en específico`
        }
...
async function executeSearchCode(params: {pattern: string, path?: string, file_extension?: string}): Promise<string> {
    ...
    const header = totalMatches >= MAX_SEARCH_RESULTS
