...
export class DevAssistantAgent {
    ...
    async chat(userMessage: string, onChunk?:(fragment: string) => void): Promise<AgentResponse> {
        this.turns++;
        ...
        while(this.toolCallsLastTurn < MAX_TOOL_CALLS) {
            ...
            if(response.stop_reason === "tool_use"){
                ...
                const results = await Promise.all(toolUseBlocks.map( async (block) => {
                    ...
                    console.log(`Ejecutando tool: ${block.name} (${JSON.stringify(block.input)} [${toolNum}/${MAX_TOOL_CALLS}])`)
    
                    const toolOuput = await executeAnyTool(block.name, block.input as Record<string, unknown>)
            ...
            const unexpectedText = response.content.filter(
                (block): block is Anthropic.Messages.TextBlock => block.type === "text"
            )
            .map((block) => block.text)
            .join("\n")
            
            const unexpectedMessage = unexpectedText || `Sesión terminada inesperadamente: ${response.stop_reason} ?? razón desconocida`;
            return {
    ...
    clearHistory(): void {
        ...
        this.turns = 0;
        this.toolCallsLastTurn = 0;
        console.log("Historial del agente limpiando")
    }
