...
export function sanitizeInput(input: string): string {
  ...
  if (result.length > MAX_LENGTH){
    result = result.slice(0, MAX_LENGTH) + 
      "\nExcedía el limite - cortando a 8000 chars"
  }
  return result
}
...
export function checkGuardrails( input: string, rateLimiter: RateLimiter): GuardrailResult {
  ...
  const injectCheck = detectPromptInjection(input);
  if(injectCheck.detected){
    console.log(`Prompt injection detectado: ${injectCheck.pattern}`)
    return {
      safe: false,
      reason: `Tu mensaje contiene patrones que intentan modificar el comportamiento del asistente. ` + 
