...
Flujo RAG
Ingestión: los archivos .md se dividen en chunks por encabezados (chunker.ts), se generan embeddings con OpenAI text-embedding-3-small y se almacenan en SQLite con sqlite-vec.
Consulta: la pregunta del usuario se embebe también con OpenAI text-embedding-3-small, se recuperan los TOP_K chunks más similares y se construye un prompt aumentado que Claude (Anthropic) usa para responder citando fuentes.
Flujo Agente
El usuario envía un mensaje; los guardrails lo validan y sanitizan.
El agente llama a Claude con las tools definidas.
Si Claude responde con tool_use, el executor invoca la herramienta y devuelve el resultado.
El ciclo se repite hasta end_turn o el límite de 8 tool calls.
Estructura del proyecto
dev-assistant/
├── src/
